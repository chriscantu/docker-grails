...
This docker image includes OpenJDK 8 and Grails 3.0.9 for use in developing Grails applications (with the Grails cli as the endpoint).
...
docker run --rm -v /path/to/your/project:/app:rw -p 80:8080 --name grails chriscantu/grails grails run-app

# For a data container
docker run --rm --link data-container-name:data -p 80:8080 --name grails chriscantu/grails grails run-app
```
...
```bash
docker run --rm -v /path/to/dir:/app:rw --name grails chriscantu/grails create-app helloworld
```
...
```bash
docker run -it --rm -v /path/to/project:/app:rw --name grails chriscantu/grails --interactive
```
...
```bash
docker build github.com/chriscantu/docker-grails
```

Or, you can pull the image from the central docker repository by using...

```bash
docker pull chriscantu/grails
```
